...
void code(void)
{
    ...
    units = src_char - (100 * huns)  - (10 * tens);     /**recuperer les unités*/
        /**former trois nouveaux variables*/
    res1 = huns + tens ;
    res2 = tens + units ;
    res3 = huns + units ;
        /**pour verifier la parité des res(i)*/
    if ((res1 % 2) == 0) par1 = 0;
    else                 par1 = 5;
    if ((res2 % 2) == 0) par2 = 0;
    else                 par2 = 5;
...
void print_code(void)       /**insere le caractere crypté dans le fichier resultat*/
{
   char lenght ;
...
void get_code(void)     /**recuper 9 caracteres quie represente un caractere crypté*/
{
    int length;
